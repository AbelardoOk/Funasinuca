...
export default function Register() {
    const [data, setData] = useState<ApiResponse<{ token: string; userName: string; cpf: string }> | null>(null);
    const [msg, setMsg] = useState("");
    const [loading, setLoading] = useState(false);
    const [token, setToken] = useLocalStorage('token', '');

    async function realizarCadastro(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
        ...
        if (nome.length < 3) {
            setMsg("Por favor, insira um nome válido (mínimo 3 caracteres).");
            setLoading(false);
            return;
        }
        ...
        try {
            // Chamada à API utilizando o método register fornecido
            const res = await userService.register(nome, cpf, email, senha);
            setData(res);
            ...
            setMsg("Erro ao realizar o cadastro. Verifique os dados ou tente novamente.");
            console.error(e);
        } finally {
            setLoading(false);
        }
        ...
        <main style={styles.page}>
            <div style={styles.card}>
                ...
                <div style={styles.header}>
                    ...
                    <p style={styles.subtitle}>Crie a sua conta gratuitamente</p>
                </div>

                {/* Form */}
                <form onSubmit={realizarCadastro} style={styles.form}>
                    ...
                    >
                        {loading ? (
                            <span style={styles.loadingContent}>
                                <span style={styles.spinner} /> Cadastrando...
                            </span>
                        ) : "Cadastrar-se"}
                    </button>
                </form>
                ...
                <p style={styles.footer}>
                    Tem conta?{" "}
                    <a href="/login" style={styles.footerLink}>Faça login</a>
                </p>
            </div>
        </main>
    );
